...
public class CommitLog {
    ...
    public long flush() {
        ...
        this.mappedFileQueue.flush(0);//ByteBuffer数据刷新到磁盘
    ...
    /**
     * Read CommitLog data, use data replication，读取数据，同步到从节点
     */
    ...
    public SelectMappedBufferResult getData(final long offset, final boolean returnFirstOnNotFound) {
        ...
        if (mappedFile != null) {
            int pos = (int) (offset % mappedFileSize);//找到offset在文件中的位置
            SelectMappedBufferResult result = mappedFile.selectMappedBuffer(pos);//从offset开始读取，直到当前文件实际内容结尾
            return result;
        }
    ...
    /**
     * When the normal exit, data recovery, all memory data have been flush  进行数据检测，如果数据不正确，则从不正确的位置，删除往后的数据
     */
    public void recoverNormally(long maxPhyOffsetOfConsumeQueue) {
        ...
        if (!mappedFiles.isEmpty()) {
            ...
            processOffset += mappedFileOffset;//[FileFromOffset, 校验正确数据Position]
    ...
    public CompletableFuture<PutMessageResult> asyncPutMessage(final MessageExtBrokerInner msg) {
        ...
        //校验消息的大小是否超出配置的大小（默认4M），将消息放到encodeBuffer
        ...
        //ReentrantLock、PutMessageSpinLock，取决于MessageStoreConfig()是否开启ReentrantLock
        ...
        try {
            ...
            if (null == mappedFile || mappedFile.isFull()) {
                //重新生成一个新的文件:计算新文件的offset，文件第一条消息的offset当作文件名
    ...
    class DefaultAppendMessageCallback implements AppendMessageCallback {
        ...
        public AppendMessageResult doAppend(final long fileFromOffset, final ByteBuffer byteBuffer, final int maxBlank,
            ...
            int pos = 4 + 4 + 4 + 4 + 4;
            // 6 QUEUEOFFSET  消息中，会存储queueOffset
